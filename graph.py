...
def get_latest_wind_speed():
    ...
    return (CSV_read["WIND_SPEED"].iloc[-1]/100)/2

def load_latest_wind_speed():
    CSV_read = pd.read_csv("sensor_data.csv", sep=",", header=0) 
    latest_wind_speed = (CSV_read["WIND_SPEED"].iloc[-1]/100)/2
...
def wind_speed_data():
    ...
    pre_Wind_speed_data = [(x / 100)/2 for x in CSV_read["WIND_SPEED"].tail(20).tolist()]
    # Eksempel data set
